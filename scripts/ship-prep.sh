...
# TODO: add `seqkit split` to regenerate individual files from starfish output
# TODO: add cleaning step for file names generated by `seqkit split`, i.e. use `rename`
# TODO: add cleaning step for starfish fasta headers generated by `seqkit split`, i.e. use `rename`

STARSHIPS="manual
starfish"

blastdb=ships/fna/blastdb/concatenated.fa
...
find ships/manual/* ships/starfish/* -type f -name "*.new" -exec cat {} + > "$blastdb"
